import logging
import os
import uuid

import boto3
from botocore.client import Config
from django.conf import settings

logger = logging.getLogger(__name__)


def get_bucket():
    config = Config(
        signature_version="s3v4",
        s3={
            "addressing_style": "path"
        },
    )

    endpoint_url = getattr(settings, "AWS_S3_ENDPOINT_URL", None)
    logger.info(f"MinIO config - Endpoint: {endpoint_url}")
    logger.info(f"MinIO config - Access Key: {settings.AWS_ACCESS_KEY_ID}")
    logger.info(f"MinIO config - Region: {settings.AWS_S3_REGION_NAME}")

    return boto3.client(
        "s3",
        aws_access_key_id=settings.AWS_ACCESS_KEY_ID,
        aws_secret_access_key=settings.AWS_SECRET_ACCESS_KEY,
        region_name=settings.AWS_S3_REGION_NAME,
        endpoint_url=endpoint_url,
        config=config,
    )


def get_presigned_url(key, method, expiration=3600):
    url_method = {"GET": "get_object", "PUT": "put_object", "DELETE": "delete_object"}.get(
        method.upper()
    )

    if not url_method:
        return None

    bucket = get_bucket()
    bucket_name = getattr(settings, "AWS_STORAGE_BUCKET_NAME", "default")
    logger.info(f"Using bucket: {bucket_name}")
    logger.info(f"Generating {method} URL for key: {key}")

    print(f"DEBUG: Bucket name: {bucket_name}")
    print(f"DEBUG: Key: {key}")
    print(f"DEBUG: Method: {url_method}")

    try:
        url = bucket.generate_presigned_url(
            ClientMethod=url_method,
            Params={"Bucket": bucket_name, "Key": key},
            ExpiresIn=expiration,
        )
        logger.info(f"Generated URL: {url}")
        return url
    except Exception as e:
        logger.error(f"Error generating presigned URL: {e}")
        raise


def generate_key(key, filename):
    name, ext = os.path.splitext(filename)
    # Remove leading dot from extension to avoid double dots
    clean_ext = ext.lstrip(".")
    return f"{key}/{uuid.uuid4()}.{clean_ext}"
